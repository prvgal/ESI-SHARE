...
#include "Viajes.h"
   ...
   /*static void cambiar_datos_veh(vehiculo_inf vehiculo){
    ...
    static int contar_vehiculos(char idusu[IDUSU]){
        FILE *file_original;
        char usuario_actual[IDUSU];
        int i;
    ...
    void proc_jose(char usu[IDUSU], viajes viaje){
        int num_veh;
        char mat[IDMAT];
        FILE *veh;
        char linea[MAX_LIN_FICHVEH];
        ...
        if(contar_vehiculos(usu)>0){
            ...
            }while(listar_vehiculos_mat(mat)==False);
            ...
            printf("\nNo es posible iniciar el viaje - no figura ningún vehículo en su cuenta.\n");
        }
    }
